// I have stolen this from goamz
// http://bazaar.launchpad.net/+branch/goamz/view/head:/aws/sign.go
...
// SignV1 signs an HTTP request utilizing version 1 of the Zero
// signature, and the given credentials.
func SignV1(req *http.Request, auth Auth) (err error) {

	queryVals := req.URL.Query()
	queryVals.Set("ZeroAccessKeyId", auth.AccessKey)
	queryVals.Set("SignatureVersion", "1")
...
func canonicalRequest(req *http.Request, sortedHdrNames []string, hasher hasher) (canReq, canReqHash string, err error) {
...
func authHeaderString(
	...
	w := new(bytes.Buffer)
	if err := errorCollector(
		fprintfWrapper(w, "Zero-HMAC-SHA256 "),
...
func requestTime(req *http.Request) (time.Time, error) {
	...
	// Get a date header.
	var date string
	if date = req.Header.Get("x-zero-date"); date == "" {
		if date = req.Header.Get("date"); date == "" {
			return time.Time{}, fmt.Errorf(`Could not retrieve date. Please provide either "x-zero-date", or "date".`)
		}
	}
	...
	return time.Time{}, fmt.Errorf("Could not parse date. Please use: %s", strings.Join(timeFormats, ","))
}
